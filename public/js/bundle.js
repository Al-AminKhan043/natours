  ...
  var login = async (email, password) => {
    try {
      ...
      if (res.data.status === "success") {
        showAlert2("success", "Loggen In Successfully");
        window.setTimeout(() => {
          location.assign("/");
        }, 1500);
      }
    } catch (e) {
      showAlert2("error", e.response.data.message);
    }
  };
  var logout = async () => {
    try {
      ...
      showAlert2("error", "Error logging out! Try again.", err);
    }
  };
    ...
    try {
      ...
      if (data.status === "success" && data.session) {
        ...
        if (checkoutUrl) {
          ...
          showAlert2("error", "Checkout URL not found!");
          console.log("Checkout URL not found in session data:", data.session);
        }
      } else {
        showAlert2("error", "Session creation failed!");
        console.log("Session Creation Failed:", data);
      }
    } catch (err) {
      console.log("Error during booking:", err);
      showAlert2("error", err.message || "Something went wrong!");
    }
  };
