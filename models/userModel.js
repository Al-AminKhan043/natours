...
userSchema.pre('save', async function (next) {
    ...
    // Remove passwordConfirm after hashing the password
    this.passwordConfirm = undefined;

    next();
});

// Set passwordChangedAt when the password is modified
userSchema.pre('save', function (next) {
    if (!this.isModified('password') || this.isNew) return next();
    this.passwordChangedAt = Date.now() - 1000;  // Set a small delay to ensure consistency
    next();
});
...
userSchema.methods.correctPassword = async function (candidatePassword, userPassword) {
    return await bcrypt.compare(candidatePassword, userPassword);  // Using bcryptjs's compare method
};

// Check if password was changed after JWT creation
userSchema.methods.changedPasswordAfter = function (JWTTimesStamp) {
    if (this.passwordChangedAt) {
        const changedTimeStap = parseInt(this.passwordChangedAt.getTime() / 1000, 10);
        return JWTTimesStamp < changedTimeStap;
